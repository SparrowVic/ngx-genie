...
export class ContentService {
  ...
  readonly faqs = computed<FaqItem[]>(() => [
    ...
    { id: 'versions', question: 'Which Angular versions are supported?', answer: 'GenieOS targets the Angular 19 line — its peer dependency is ^19. Older majors stay maintained on long-lived support branches, so Angular 18 through 22 are supported — each published as its own version-tagged release on npm.', tag: 'Compatibility' },
  ...
  readonly roadmap = signal<RoadmapPhase[]>([
    ...
    { quarter: 'Now', title: 'Angular 19 line', status: 'in-progress', items: [{ text: 'DI-internals fixes', done: true }, { text: 'Compatibility test suite', done: true }, { text: 'Enterprise graph perf', done: true }, { text: 'Export filtered tree to JSON', done: true }, { text: '19.0.0-beta.3 on npm', done: false }] },
